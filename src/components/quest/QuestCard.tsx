...
import { formatLocalDate, getTimeStatus } from "@/utils/dateUtils";
import {
...
// ============================================================================
// UTILS this could be imported from utils
// ===========================================================================
...
const QuestBanner: React.FC<{ quest: any; now: Date }> = ({ quest, now }) => {
  ...
  const isCanceled =
    quest.status?.toLowerCase() === "canceled" ||
    quest.status?.toLowerCase() === "cancelled";
  ...
  // Modern status badge design for all state
    ...
    <div className="w-full relative">
      {isPlaceholder ? (
        ...
        <Image
          ...
          onError={(e) => {
            console.warn(
              "❌ Image failed to load, showing fallback placeholder"
            );
            // Create a fallback placeholder element
            const placeholder = document.createElement("div");
            placeholder.className =
              "w-full h-[240px] rounded-lg bg-gradient-to-br from-[#2C2C30] via-[#1E1E20] to-[#161618] border border-[#44444A] flex items-center justify-center";
      ...
      >
        <div className="flex items-center gap-1 sm:gap-2">
          <div
            className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${statusDesign.dot}`}
          />
          <span
            className={`text-sm sm:text-lg font-bold ${statusDesign.text} capitalize`}
          >
            {statusDesign.label}
          </span>
        </div>
    ...
    <div className="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3 lg:gap-4">
      ...
      <div className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl bg-purple-500/10 border border-purple-400/30 backdrop-blur-sm transition-all duration-300 shrink-0">
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="flex flex-col">
            <span className="text-xs leading-none text-[#ACB5BB] hidden sm:block">
              Start Date
            </span>
            <span className="font-mono text-sm sm:text-lg font-bold leading-none text-purple-300">
              {quest.startDateTime
                ? formatLocalDate(quest.startDateTime)
                : "--"}
            </span>
          </div>
        </div>
...
const getTaskIcon = (task: string) => {
  ...
  return (
    <Twitter className="w-4 h-4 sm:w-5 sm:h-5 text-white" strokeWidth={2} />
  );
};
...
const ParticipantsProgressBar: React.FC<{ quest: any }> = ({ quest }) => {
  ...
  // dinacmic colors based on progress
...
const QuestCard: React.FC<QuestCardProps> = memo(
  ({ quest, now, onOpenModal, buttonText, buttonDisabled }) => {
    return (
