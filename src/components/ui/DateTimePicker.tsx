...
const generateTimeOptions = (
  ...
  console.log(
    `⏰ [DateTimePicker] Generated ${times.length} time options starting from ${times[0]}`
  );
  return times;
};
  ...
  return (
    ...
    >
      {dates.map((date) => {
        ...
        const today = new Date().toLocaleDateString("sv-SE"); // "YYYY-MM-DD" en local
...
const DateTimePicker: React.FC<DateTimePickerProps> = ({
  ...
  useEffect(() => {
    const now = new Date();
    console.log(
      "🌍 [DateTimePicker] Current local time:",
      now.toLocaleString()
    );
    console.log("🌍 [DateTimePicker] Current UTC time:", now.toISOString());
    console.log(
      "🌍 [DateTimePicker] Timezone offset:",
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <svg
                className={`w-4 h-4 text-[#ACB5BB] transition-transform duration-200 ${
