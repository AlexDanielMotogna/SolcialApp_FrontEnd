...
export async function POST(req: NextRequest) {
  ...
  if (!username || !email || !password || !phone) {
    return new Response(JSON.stringify({ msg: "missing_required_fields" }), {
      status: 400,
    });
  }
  ...
  if (existing) {
    return new Response(JSON.stringify({ msg: "user_already_exists" }), {
      status: 409,
    });
  }
  ...
  return new Response(
    JSON.stringify({ status: "success", msg: "user_registered_verify_email" }),
    { status: 200 }
  );
}
