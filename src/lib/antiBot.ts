...
//not integrated yet
// This function verifies the reCAPTCHA token using Google's API
/* export async function verifyRecaptcha(token: string): Promise<{
  ...
  try {
    ...
    // For testing, we can lower the threshold to allow more requests
    const minScore = 0.3; // Bajado de 0.5 a 0.3 para testing
    if (data.score < minScore) {
      return {
        success: false,
        error: `Low security score: ${data.score}`,
        score: data.score,
      };
    }
...
/* export async function validateRequest(
  ...
  console.log(
    `✅ reCAPTCHA verified successfully - Score: ${captchaResult.score}`
  );
  ...
  if (ipRateLimit.isBlocked) {
    return {
      ...
      error: `Too many attempts. Please wait ${Math.ceil(
        ipRateLimit.remainingTime / 1000
      )} seconds.`,
      statusCode: 429,
    };
  }
  ...
  const walletRateLimit = rateLimit(
    `quest_join_wallet_${walletaddress}`,
    5,
    600000
  ...
  if (walletRateLimit.isBlocked) {
    return {
      valid: false,
      error: "Too many attempts from this wallet",
      statusCode: 429,
    };
  }
